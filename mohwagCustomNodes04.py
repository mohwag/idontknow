...
class mwFullPipe_KSAStart:
    ...
    #RETURN_TYPES = ("LATENT", "IMAGE", "INT", "INT", "INT")
    #RETURN_NAMES = ("latent", "image", "noise_seed", "end_at_step", "steps")
    RETURN_TYPES = ("MWFULLPIPE", "MWLTNTPIPE", "LATENT", "IMAGE", "INT", "INT", "INT", "INT", "INT")
    RETURN_NAMES = ("fullPipe", "ltntPipe", "latent", "image", "noise_seed", "end_at_step", "steps", "wd", "ht")
    FUNCTION = "sample"
    CATEGORY = "mohwag"

    #def sample(self, mwFullPipe, latent_image, noise_seed, positive, negative, cfg, start_at_step, end_at_step, steps, add_noise, return_with_leftover_noise, denoise=1.0):
    def sample(self, mwFullPipe, noise_seed, positive, negative, cfg, end_at_step, steps, add_noise, return_with_leftover_noise, startW, cropW, offsetW, outputW, startH, cropH, offsetH, outputH, cropToOutput_upscaleMethod, denoise=1.0, ):
        #mwCkptPipe, noise_seed, sampler_name, scheduler = mwFullPipe
        start_at_step = 0
        ...
        if add_noise == "disable":
            disable_noise = True
        ltnt = common_ksampler(model1c, noise_seed, steps, cfg, sampler_name, scheduler, positive, negative, latentDictTuple[0], denoise=denoise, disable_noise=disable_noise, start_step=start_at_step, last_step=end_at_step, force_full_denoise=force_full_denoise)
        ltntOut = (ltnt, )
        ...
        return (mwFullPipe, ltntPipe, ltntUpscaleOut[0], imgOut[0], noise_seed, end_at_step, steps, outputW, outputH)
        #return (ltntOut[0], imgOut[0], noise_seed, end_at_step, steps)
...
class mwFullPipe_KSA:
    ...
    def sample(self, mwFullPipe, ltntPipe, positive, negative, noise_seed, cfg, start_at_step, end_at_step, steps, add_noise, return_with_leftover_noise, multOver8, upscale_method, denoise=1.0, ):
        ...
        if add_noise == "disable":
            disable_noise = True
        runLtnt = common_ksampler(model1c, noise_seed, steps, cfg, sampler_name, scheduler, positive, negative, scaleLtntOut[0], denoise=denoise, disable_noise=disable_noise, start_step=start_at_step, last_step=end_at_step, force_full_denoise=force_full_denoise)
        runLtntOut = (runLtnt, )
        ...
        return (mwFullPipe, ltntPipe, runLtntOut[0], imgOut[0], noise_seed, end_at_step, steps, outputW, outputH)
