...
class mwCalcTileRowsAndCols():
    @classmethod
    def INPUT_TYPES(moh):
        return {"required": {
                    "sPipeC": ("MWSIZEPIPE",),            
                    "cols": ("INT", {"default": 1, "min": 0, "max": 16, "step": 1}),
                    "rows": ("INT", {"default": 1, "min": 0, "max": 16, "step": 1}),
                    }}
    RETURN_TYPES = ("MWTILEPIPE", "MWSIZEPIPE")
    RETURN_NAMES = ("tilePipe", "bounded_sPipeC")
    FUNCTION = "mwCTRAC"
    CATEGORY = "mohwag/Conditioning"
    def mwCTRAC(self, sPipeC, cols, rows):
        tilePipe_out = sPipeC, cols, rows
        w, h = sPipeC
        bounded_sPipeC = int(w / cols), int(h / rows)
        return(tilePipe_out, bounded_sPipeC)

class mwConditioningCnetTileAll():
    @classmethod
    def INPUT_TYPES(moh):
        return {"required": {
                    "condPos": ("CONDITIONING",),
                    ...
                    #"cols": ("INT", {"default": 1, "min": 0, "max": 16, "step": 1}),
                    #"rows": ("INT", {"default": 1, "min": 0, "max": 16, "step": 1}),
                    "refImgsPixelMultPos": ("FLOAT", {"default": 1.00, "min": 0.00, "max": 25.00, "step": 0.05}),
                    "refImgsPixelMultNeg": ("FLOAT", {"default": 1.00, "min": 0.00, "max": 25.00, "step": 0.05}),
                    "overlap_xpndPct": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.01}),
    ...
    FUNCTION = "mwCSAA"
    CATEGORY = "mohwag/Conditioning"
    def mwCSAA(self, condPos, condNeg, control_net, image, tilePipe, refImgsPixelMultPos, refImgsPixelMultNeg, overlap_xpndPct, cond_strength, cnet_interval_strength):
        sPipeC, cols, rows = tilePipe
        wC, hC = sPipeC
        #halfOverlap = overlap //2
        wi0 = int(wC / cols)
        hi0 = int(hC / rows)

        geoAvgLen = int(((wi0) * (hi0))**(1/2))
        ratio = round(wi0 / hi0, 2)
        metrics_out = wi0, hi0, geoAvgLen, ratio
        ...
        intervalList = isList[0].split(",",-1)
        intervalList = [float(x) for x in intervalList]
        strengthList = isList[1].split(",",-1)
        ...
        for i in range(cols):
            ...
            x1_0 = (i+1) * wi0
            w0 = x1_0 - x0_0
            x0_1, x1_1 = psPipeModForLtnt(wC, x0_0 - (w0 * overlap_xpndPct), x1_0 + (w0 * overlap_xpndPct), 1)

            for j in range(rows):
                y0_0 = j * hi0
                y1_0 = (j+1) * hi0
                h0 = x1_0 - x0_0
                y0_1, y1_1 = psPipeModForLtnt(hC, y0_0 - (h0 * overlap_xpndPct), y1_0 + (h0 * overlap_xpndPct), 1)
                
                psPipeC_ij = int(x0_1), int(y0_1), int(x1_1 - x0_1), int(y1_1 - y0_1)
                sPipeC_ij = int(x1_1 - x0_1), int(y1_1 - y0_1)

                condPosCopy = condPos.copy()
                start_width, start_height, cropW, cropH, targetW, targetH = sdxl_cond_sizes(sPipeC_ij, refImgsPixelMultPos)
                for i in range(len(condPosCopy)):
                    condPosCopy[i][1]["width"] = start_width
                    condPosCopy[i][1]["height"] = start_height
                    condPosCopy[i][1]["crop_w"] = cropW
                    condPosCopy[i][1]["crop_h"] = cropH
                    condPosCopy[i][1]["target_width"] = targetW
                    condPosCopy[i][1]["target_height"] = targetH


                condNegCopy = condNeg.copy()
                start_width, start_height, cropW, cropH, targetW, targetH = sdxl_cond_sizes(sPipeC_ij, refImgsPixelMultNeg)
                for i in range(len(condNegCopy)):
                    condNegCopy[i][1]["width"] = start_width
                    condNegCopy[i][1]["height"] = start_height
                    condNegCopy[i][1]["crop_w"] = cropW
                    condNegCopy[i][1]["crop_h"] = cropH
                    condNegCopy[i][1]["target_width"] = targetW
                    condNegCopy[i][1]["target_height"] = targetH


                pnAreaCond_ij = mwpsConditioningSetAreaMethod(condPosCopy, condNegCopy, psPipeC_ij, cond_strength)
                boundedImage_ij = mwImageCropMethod(image, psPipeC_ij)[0]

                for k in range(len(strengthList)):
                    if k == 0:
                        startAdd = 0.0
                    else:
                        startAdd = 0.001

                    pnOut_ijk = controlNetApplyAdvancedMethod(pnAreaCond_ij[0], pnAreaCond_ij[1], control_net, boundedImage_ij, strengthList[k], intervalList[k] + startAdd, intervalList[k+1])
                    condPosF = conditioningCombineMethod(condPosF, pnOut_ijk[0])[0]
                    condNegF = conditioningCombineMethod(condNegF, pnOut_ijk[1])[0]
...
class mwpsScale_LtntConform:
    ...
    RETURN_TYPES = ("IMAGE", "IMAGE", "MASK", "MASK", "MWSIZEPIPE", "MWPOSSIZEPIPE")
    RETURN_NAMES = ("images_upsc", "boundedImgs_upsc", "mask_upsc", "boundedMask_upsc", "sPipe", "psPipe")
    FUNCTION = "mwPSSLC"
    CATEGORY = "mohwag/scaling"
    def mwPSSLC(self, images, mask, psPipe, multOver8, nextMO8, upscale_method):
        ...
        imgWu = imgW * nextMO8 / multOver8
        imgHu = imgH * nextMO8 / multOver8
        x0u = x0 * nextMO8 / multOver8
        x1u = x1 * nextMO8 / multOver8
        y0u = y0 * nextMO8 / multOver8
        y1u = y1 * nextMO8 / multOver8

        imgWuf = int(imgWu)
        imgHuf = int(imgHu)
        x0uc, x1uc = psPipeModForLtnt(imgWu, x0u, x1u)
        y0uc, y1uc = psPipeModForLtnt(imgHu, y0u, y1u)
        ...
        return (img_u, boundedImage_u, mask_u, boundedMask_u, s_ucf, ps_ucf)
"""
...
"mwCalcTileRowsAndCols": mwCalcTileRowsAndCols,
"mwConditioningCnetTileAll": mwConditioningCnetTileAll,
...
"mwCalcTileRowsAndCols": "CalcTileRowsAndCols",
"mwConditioningCnetTileAll": "ConditioningCnetTileAll",
