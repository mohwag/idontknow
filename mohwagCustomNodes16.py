...
class mwCkpt_ckptMerge:
    ...
    def mwCCM(self, mwCkpt1, mwCkpt2, wgt_Ckpt1):
        ...
        model1, clip1, vaeAlways = mwCkpt1[:3]
        model2, clip2, _ = mwCkpt2[:3]

        m = model1.clone()
        kp = model2.get_key_patches("diffusion_model.")
        for k in kp:
            m.add_patches({k: kp[k]}, 1.0 - wgt_Ckpt1, wgt_Ckpt1)
        n = clip1.clone()
        lp = clip2.get_key_patches()
        for l in lp:
            if l.endswith(".position_ids") or l.endswith(".logit_scale"):
                continue
            n.add_patches({l: lp[l]}, 1.0 - wgt_Ckpt1, wgt_Ckpt1)
...
class mwLora_LoadAdv: #comfy nodes.py LoraLoader
    @classmethod
    def INPUT_TYPES(moh):
        return {"required": { 
                              "lora_name": (folder_paths.get_filename_list("loras"), ),
                              "clipLastLayer": ("INT", {"default": -2, "min": -128, "max": 128, "step": 1 }),
                              }}
    RETURN_TYPES = ("MWLORA",)
    RETURN_NAMES = ("mwLora",)
    FUNCTION = "mwLLA"
...
class mwLtntPipe_rmvMask:
    ...
    def mwLTRM(ltntPipe):
        ...
        ltntNew = ltnt.copy()
        #ltntNew["noise_mask"] = None
        del ltntNew["noise_mask"]

        ltntPipeNew = ltntNew, startW, startH, inputW, inputH
...
"mwLtntPipe_rmvMask": mwLtntPipe_rmvMask,
}
...
"mwLtntPipe_rmvMask": "LtntPipe_rmvMask",
}