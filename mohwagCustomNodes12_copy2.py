...
class mwCondXLa:
    ...
    def mwCXLa(self, clip, compType, condText, width, height):
        condListF = prepString(condText)
        ...
        rats = round((np.exp(1))**(np.log(width / height) * (2/3)),2)
...
class mwCompScale:
    ...
    def mwCS(self, x, y, w, h, multOver8, nextMO8):
        xout = int(x * nextMO8 / 8 / multOver8)
        yout = int(y * nextMO8 / 8 / multOver8)
        wout = int(w * nextMO8 / 8 / multOver8)
        hout = int(h * nextMO8 / 8 / multOver8)
...
class mwMaskBoundingBox:
    ...
    def INPUT_TYPES(moh):
        return {
            "required": {
                ...
                "multOver8": ("INT", {"default": 8, "min": 1, "max": 64, "step": 1}),                
                "threshold": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.01}),
                "scalingLimit": (["8ths", "4ths", "halfs", "whole"],),
            }
        }
    ...
    def mwMBB(self, mask_bounding_box, image_mapped, multOver8, threshold, scalingLimit):

        base_unit_size = 64
        if scalingLimit == "halfs":
            base_unit_size = 16
        elif scalingLimit == "4ths":  
            base_unit_size = 32
        elif scalingLimit == "whole":
            base_unit_size = 8
