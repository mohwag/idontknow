...
class mwLtntPipe_Create: #WAS_Latent_Size_To_Number
    ...
    def INPUT_TYPES(moh):
        return {"required": {
            "ltnt": ("LATENT", ),
            "initialW": ("INT", {"default": 512, "min": 64, "max": 2048, "step": 64 }),
            "initialH": ("INT", {"default": 512, "min": 64, "max": 2048, "step": 64 }),
            }}
    ...
    def cnvt(self, ltnt, initialW, initialH):
        size_dict = {}
        i = 0
...
class mwCond:
    ...
    def condSetup(self, compType, condText):
...
class mwFullPipe_KSAStart:
    ...
    def INPUT_TYPES(moh):
        return {"required":
                    ...
                    "optional": {
                        "mwCondPosComb": ("MWCOND", ),
                        "mwCondNegComb": ("MWCOND", ),
                    }
                }

    RETURN_TYPES = ("MWFULLPIPE", "MWLTNTPIPE", "MWCOND", "MWCOND", "MWCOND", "MWCOND", "INT", "INT", "INT", "LATENT", "IMAGE", "INT", "INT")
    RETURN_NAMES = ("fullPipe", "ltntPipe", "mwCondPos", "mwCondNeg", "mwCondPosComb", "mwCondNegComb", "noise_seed", "end_at_step", "steps", "latent", "image", "wd", "ht")
    FUNCTION = "sample"
    CATEGORY = "mohwag"
    ...
    def sample(self, mwFullPipe, noise_seed, mwCondPos, mwCondNeg, cfg, end_at_step, steps, add_noise, return_with_leftover_noise, startW, cropW, offsetW, outputW, startH, cropH, offsetH, outputH, cropToOutput_upscaleMethod, posCombStrn, negCombStrn, mwCondPosComb = (), mwCondNegComb = (), denoise=1.0):

        start_at_step = 0
        
        mwCkptPipe, sampler_name, scheduler = mwFullPipe
        ckptPipe1b, ckptPipe1c = mwCkptPipe
        model1c, clip1c, _ = ckptPipe1c
        vaeAlways = ckptPipe1b[2]

        positive = mwCondPrep(clip1c, mwCondPos)[0]
        negative = mwCondPrep(clip1c, mwCondNeg)[0]
        ...
        if mwCondPosComb and posCombStrn != 0:
            posComb = mwCondPrep(clip1c, mwCondPosComb)[0]
            posCombWgtd = []
            for t in posComb:
                n = [t[0], t[1].copy()]
        ...
        if mwCondNegComb and negCombStrn != 0:
            negComb = mwCondPrep(clip1c, mwCondNegComb)[0]
            negCombWgtd = []
            for t in negComb:
                n = [t[0], t[1].copy()]
                n[1]['area'] = (startH // 8, startW // 8, 0, 0)
        ...
        return (mwFullPipe, ltntPipe, mwCondPos, mwCondNeg, mwCondPosComb, mwCondNegComb, noise_seed, end_at_step, steps, ltntUpscaleOut[0], imgOut[0], outputW, outputH)
...
class mwFullPipe_KSA:
    ...
    def sample(self, mwFullPipe, ltntPipe, mwCondPos, mwCondNeg, noise_seed, cfg, start_at_step, end_at_step, steps, add_noise, return_with_leftover_noise, multOver8, upscale_method, posCombStrn, negCombStrn, mwCondPosComb = (), mwCondNegComb = (), denoise=1.0):
        ...
        return (mwFullPipe, ltntPipe, mwCondPos, mwCondNeg, mwCondPosComb, mwCondNegComb, noise_seed, end_at_step, steps, runLtntOut[0], imgOut[0], outputW, outputH)
