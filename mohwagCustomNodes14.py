...
from nodes import MAX_RESOLUTION, ConditioningSetArea
...
class mwLtntPipe_CropScale: #WAS_Latent_Size_To_Number
    ...
    RETURN_TYPES = ("MWLTNTPIPE", "MWSIZEPIPE", "MWPOSSIZEPIPE")
    RETURN_NAMES = ("ltntPipe", "sPipe", "psPipe")
    FUNCTION = "mwLPCS"
    CATEGORY = "mohwag/LatentPipe"
    ...
    def mwLPCS(self, ltntPipe, cropLft_units, cropRgt_units, TotDeltaW_units, cropTop_units, cropBtm_units, TotDeltaH_units, cropToOutput_upscaleMethod):
        ...
        return (ltntPipeNew, outputS, outputPS)
...
class mwCondXLa:
    ...
    def INPUT_TYPES(moh):
        return {"required": {
            "clip": ("CLIP", ),
            "compType": (moh.compositionTypes, {"default": "textConcat"}),
            "refImgsPixelMult": ("FLOAT", {"default": 0.5, "min": 0.5, "max": 16, "step": 0.5}),  
            "condText": ("STRING", {"multiline": True}),
    ...
    RETURN_NAMES = ("conditioning", "clip_pt", "actualS_pt", "condText_pt")
    FUNCTION = "mwCXLa"
    CATEGORY = "mohwag/Conditioning"

    def mwCXLa(self, clip, compType, refImgsPixelMult, condText, actualS):
        condListF = prepString(condText)
        ...
        return (condReturn[0], clip, actualS, condText)
...
class mwFullPipe_KSAStart:
    ...
    def mwFPKSAS(self, mwFullPipe, positive, negative, cfg, stepEnd, steps, seed_deltaVsOrig, add_noise, return_with_leftover_noise, startS, denoise=1.0):
        stepStart = 0
        ...
        return (mwFullPipe, ltntPipe, positive, negative, stepEnd, steps, ltntOut[0], imgOut[0], startS)
...
class mwFullPipe_KSA:
    ...
    RETURN_TYPES = ("MWFULLPIPE", "MWLTNTPIPE", "CONDITIONING", "CONDITIONING", "INT", "INT", "LATENT", "IMAGE", "MWSIZEPIPE", "MWSIZEPIPE")
    RETURN_NAMES = ("fullPipe", "ltntPipe", "positive", "negative", "stepEnd", "steps", "latent", "image", "sPipe", "init_sPipe")
    FUNCTION = "mwFPKSA"
    CATEGORY = "mohwag/Sampling"
    ...
    def mwFPKSA(self, mwFullPipe, ltntPipe, positive, negative, cfg, stepStart, stepEnd, steps, seed_deltaVsOrig, add_noise, return_with_leftover_noise, multOver8, upscale_method, denoise=1.0):
        ...
        outputS = outputW, outputH
        outputS_init = startW, startH
        return (mwFullPipe, ltntPipe, positive, negative, stepEnd, steps, runLtntOut[0], imgOut[0], outputS, outputS_init)
...
class mwNumIntx64s:
    ...
    def INPUT_TYPES(moh):
        return {
            "required": {
                "width": ("INT", {"default": 0, "min": -10000, "max": 10000, "step": moh.numMult}),
                "height": ("INT", {"default": 0, "min": -10000, "max": 10000, "step": moh.numMult}),
            }
        }
...
class mwsPipe_NextMO8:
    ...
    def INPUT_TYPES(moh):
        return {
            "required": {
                "init_sPipe": ("MWSIZEPIPE",),
                "nextMO8": ("INT", {"default": 8, "min": 0, "max": 64, "step":1}),
            }
        }
    RETURN_TYPES = ("INT", "MWSIZEPIPE",)
    RETURN_NAMES = ("nextMO8", "NextsPipe",)
    FUNCTION = "mwNSP"
    CATEGORY = "mohwag/other"

    def mwNSP(self, init_sPipe, nextMO8):
        initw, inith = init_sPipe
