...
#pulling in funcs
from nodes import MAX_RESOLUTION, ConditioningSetArea, CheckpointLoaderSimple, ImageScale, EmptyLatentImage, ConditioningCombine, ConditioningAverage, ConditioningConcat, ConditioningSetTimestepRange, ConditioningSetAreaStrength, CLIPSetLastLayer, common_ksampler
...
class mwGridOverlay:
    ...
    def INPUT_TYPES(moh):
        return {
            "required": {
                ...
                "transparency": ("INT", {"default": 20, "min": 10, "max": 100, "step": 5})
            },
            "optional": {
    ...
    RETURN_NAMES = ("image_pt", "sPipeStart_pt", "image_wGrid")
    FUNCTION = "mwGO"
    CATEGORY = "mohwag/manip"

    def mwGO(self, image, scndSpacing, primSpacMult, scndThickness, primThickMult, lineColor, transparency, sPipeStart = None):
        ...
        if sPipeStart != None:
            startW, _ = sPipeStart
            scndSpacing = int(scndSpacing * w / startW)
        ...
        return (image, sPipeStart, outImg_wGrid)
...
class mwMaskSegment:
    ...
    def mwMS(self, sPipeCurr, invert, unitVal, deltaLft_units, deltaRgt_units, deltaTop_units, deltaBtm_units, unitVal_feath, feathLft_units, feathRgt_units, feathTop_units, feathBtm_units):
...
class mwMaskSegmentByPS:
    ...
    def mwMSBPS(self, sPipeCurr, psPipe_mask, invert):
        
        backW, backH = sPipeCurr
        backMask = solidMaskMethod(0.0, backW, backH)[0]

        frontX, frontY, frontW, frontH = psPipe_mask

        frontMask = solidMaskMethod(1.0, frontW, frontH)[0]
        
        combMask = maskCompositeMethod(backMask, frontMask, frontX, frontY, "add")[0]
...
class mwImageCropwParams:
    ...
    RETURN_TYPES = ("IMAGE", "MWSIZEPIPE", "MWPOSSIZEPIPE")
    RETURN_NAMES = ("image", "sPipe", "psPipe")
    FUNCTION = "mwICWP"
    CATEGORY = "mohwag/image"
    def mwICWP(self, image, unitVal, deltaLft_units, deltaRgt_units, deltaTop_units, deltaBtm_units):
        w, h = mwImageTo_sPipeMethod(image)[0]
        ...
        out_sPipe = (w, h)
        out_psPipe = (x, y, w, h)
        return (output, out_sPipe, out_psPipe)
...
class mwImageConform_StartSizeXL:
    ...
    RETURN_NAMES = ("image", "sPipeStart_updtd")
    FUNCTION = "mwICSSXL"
    CATEGORY = "mohwag/image"
...
class mwpsConditioningSetArea():
    ...
    def mwCSA(self, condPos, condNeg, psPipe, strength):
        ...
        for i in range(len(condPosList)):
            newVal = (condPosList[i][1].get("strength")) * strengthMult
            condPosList[i][1]["strength"] = newVal
            condPosList[i][1]["area"] = (h//8, w//8, y//8, x//8)
            condPosList[i][1]["set_area_to_bounds"] = False

        for i in range(len(condNegList)):
            newVal = (condNegList[i][1].get("strength")) * strengthMult
            condNegList[i][1]["strength"] = newVal
...
class mwLtntPipe_Create:
    ...
    def mwLPC(self, ltnt, sPipeStart):
        initialW, initialH = sPipeStart
        size_dict = {}
        i = 0
...
class mwLtntPipe_Create2:
    ...
    FUNCTION = "mwLPC"
    CATEGORY = "mohwag/LatentPipe"
    def mwLPC(self, image, mwFullPipe, sPipeStart):
        initialW, initialH = sPipeStart
        ...
        outputH = 8 * float(size_dict[0][1])
        '''
        outputW, outputH = mwImageTo_sPipeMethod(image)[0]
        return ((ltnt, initialW, initialH, outputW, outputH),)
...
class mwLtntPipe_CropScale:
    ...
    RETURN_NAMES = ("ltntPipe", "sPipe", "psPipe", "sPipeStart_updtd")
    FUNCTION = "mwLPCS"
    CATEGORY = "mohwag/manip"
...
class mwImage_VertSqzExpnd:
    ...
    RETURN_TYPES = ("IMAGE", "MWSIZEPIPE", "MWSIZEPIPE", "MWIVSEPIPE")
    RETURN_NAMES = ("image", "sPipe", "sPipeStart_updtd", "ivsePipe")
    FUNCTION = "mwIVSE"
    CATEGORY = "mohwag/manip"
    def mwIVSE(self, image, sPipeStart, yTop, yBtm, sqzDelta_units, scaleMethod):
        modImage = Image.fromarray(np.clip(255. * image.cpu().numpy().squeeze(), 0, 255).astype(np.uint8))
        inputW = modImage.size[0]
        inputH = modImage.size[1]
        
        if sqzDelta_units == 0:
            return (image, (inputW, inputH), sPipeStart, (image, sPipeStart, yTop))
        ...
        out_ivsePipe = (addBtmImg, newSPipeStart, yTop)
        return (addBtmImg, outputS, newSPipeStart, out_ivsePipe)

mwImageVertSqzExpndMethod = mwImage_VertSqzExpnd().mwIVSE  #self, image, sPipeStart, yTop, yBtm, sqzDelta_units, scaleMethod)
...
class mwImage_VertSqzExpndStack:
    ...
    FUNCTION = "mwIVSES"
    CATEGORY = "mohwag/manip"
    def mwIVSES(self, ivsePipe, yTop, sqzDelta_units, scaleMethod):
        image, sPipeStart, yBtm = ivsePipe

        return mwImageVertSqzExpndMethod(image, sPipeStart, yTop, yBtm, sqzDelta_units, scaleMethod)
'''
modImage = Image.fromarray(np.clip(255. * image.cpu().numpy().squeeze(), 0, 255).astype(np.uint8))
inputW = modImage.size[0]
inputH = modImage.size[1]
...
if sqzDelta_units == 0:
    return (image, (inputW, inputH), sPipeStart, ivsePipe)

startW, startH = sPipeStart

unitVal = int(8 * inputW / startW)
yTop = int(unitVal * (yTop / 8))
yBtm = int(unitVal * (yBtm / 8))

sqzVertRng_units = int((yBtm - yTop) // unitVal)
...
class mwLtntPipeBranch1: #WAS_Latent_Size_To_Number
    ...
    RETURN_TYPES = ("LATENT",)
    RETURN_NAMES = ("latent", "sPipeStart", "sPipeCurr")
    FUNCTION = "mwLPBO"
    CATEGORY = "mohwag/LatentPipe"
...
"mwVaeBranch_ckpt": mwVaeBranch_ckpt,
...
"mwVaeBranch_ckpt": "VaeBranch_ckpt",
