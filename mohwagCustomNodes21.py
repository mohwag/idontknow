...
#pulling in funcs
from nodes import MAX_RESOLUTION, ConditioningSetArea, CheckpointLoaderSimple, ImageScale, EmptyLatentImage, ConditioningCombine, ConditioningAverage, ConditioningConcat, ConditioningSetTimestepRange, ConditioningSetAreaStrength, common_ksampler
...
conditioningSetTimestepRangeMethod = ConditioningSetTimestepRange().set_range  #(self, conditioning, start, end)
...
class mwGridOverlay:
    ...
    def mwGO(self, image, scndSpacing, primSpacMult, scndThickness, primThickMult, lineColor, transparency, sPipeStart = None):
        ...
        if sPipeStart != None:
            startW, _ = sPipeStart
            scndSpacing = int(scndSpacing * startW / w)
...
class mwLtntPipe_VertSqzExpnd:
    ...
    RETURN_NAMES = ("ltntPipe", "sPipe", "sPipeStart_updtd")
    FUNCTION = "mwLPVS"
    CATEGORY = "mohwag/manip"

    def mwLPVS(self, ltntPipe, y, sqzVertRng_units, sqzDelta_units, scaleMethod):
        ...
        ltntMdlSqz['samples'] = comfy.utils.common_upscale(ltntMdl["samples"], x1//8, sqzH//8, scaleMethod, "disabled")
...
class mwImage_VertSqzExpnd:
    ...
    RETURN_NAMES = ("image", "sPipe", "sPipeStart_updtd")
    FUNCTION = "mwLPVS"
    CATEGORY = "mohwag/manip"
...
def cuString(aString:str) -> str:
    ...
    return aString.replace("    ", " ",-1).replace("   ", " ",-1).replace("  ", " ",-1).replace("  ", " ",-1)
...
def prepString(aBlock:str) -> list[str]:
    ...
    for x in aList:
        athingy = x.split("||")
        condListL.append(athingy[0])
        condListR.append(athingy[-1])
